...
from PointSearch.sites.zhihu.es_zhihu import ZhiHuQuestionIndex, ZhiHuAnswerIndex
from PointSearch.utils.common import extract_num, extract_num_include_comma, real_time_count
from PointSearch.utils.es_utils import generate_suggests
from PointSearch.utils.mysql_utils import fun_sql_insert
from PointSearch.utils.string_util import exclude_none
...
class ZhiHuQuestionItem(scrapy.Item, MysqlItem, ElasticSearchItem):
    ...
    def clean_data(self):
        ...
        if len(self["watch_user_num"]) == 2:
            ...
            self["watch_user_num"] = extract_num_include_comma(watch_user_num_click[0])
            self["click_num"] = extract_num_include_comma(watch_user_num_click[1])
        else:
            watch_user_num_click = self["watch_user_num"]
            self["watch_user_num"] = extract_num_include_comma(watch_user_num_click[0])
            self["click_num"] = 0
